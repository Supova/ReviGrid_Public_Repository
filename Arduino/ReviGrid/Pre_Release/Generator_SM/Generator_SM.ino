...
// AIChan -> genVoltPin
byte genVoltPin = A5; // This analog input channel reads the generator voltage
// AIChan6 -> genVoltDropPin
byte genVoltLowPin = A6; // reads the lower voltage from the other side of 47 ohm resistor
...
char *commands[] = {"init","runRange", "trackOn","trackOff","motOn","motOff",
                    "setLoad>1","setMot>1","setKp>1","setKi>1","setKd>1","setR>1","setG>1","setB>1", "setPWM>2",
                    "getVolts<1", "getBV<2",
                    "getVal<1","getKW<1","getCarbon<1","getAll<7","getRes<1","getDrop<1","getCurrent<1","getBusy<1",
                    "off","eoc"};
...
void setup() {
  Serial.begin(115200);
//  TCCR0B = (TCCR0B & 0b11111000) | 0x05; // CS01 bit set
  checkSurfaceMount();
...
void checkSurfaceMount(){
  pinMode(A0,INPUT_PULLUP);
  byte SM=digitalRead(A0);
  if(!SM){
