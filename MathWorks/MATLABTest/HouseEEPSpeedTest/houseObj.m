    ...
    methods
        ...
        function setUpCmds(obj)
            ...
            while strcmp('eoc', cmdName) == 0
                numOfOutput = 0;
                numOfInput = 0;
                specialIndex = -1; % index of special char
                                   % ">" means cmd needs input
                                   % "<" means cmd has output
                firstSpecialIndex = inf;
                % if name contains ">"
                if contains(cmdName, ">") == 1
                    % update num of output
                    specialIndex = strfind(cmdName, ">");
                    firstSpecialIndex = specialIndex;
                    if contains(cmdName, "<") == 1
                        numOfOutput = extractBetween(cmdName, ">", "<");
                    else
                ...
                if contains(cmdName, "<") == 1
                    ...
                    specialIndex = strfind(cmdName, "<");
                    if specialIndex < firstSpecialIndex
                        firstSpecialIndex = specialIndex;
                    end
                ...
                if specialIndex ~= -1
                    % update name
                    cmdName = cmdName(1:firstSpecialIndex - 1);
                end
        ...
        function isValid = checkValid(obj, currCmd, input)
            ...
            if(input < currCmd.minRange || input > currCmd.maxRange)
                flag = 0;
                str = "ERROR: Input out of range, try again."
                error(str);
            end
            isValid = flag;
        end % isvalid()
        ...
        function sendCmdArgument(obj, currCmd, args)
        ...
        function returnedVal = sendAndReadCmd(obj, currCmd, args)
            if numel(args) ~= currCmd.out
                str = "ERROR: Too many arguments.";
                error(str);
            end
