...
voltDiv = sendread(osci, 'C1:VDIV?');
voltDiv = str2double(voltDiv(8:end-1));
voltOffset = sendread(osci, 'C1:OFST?');
voltOffset = str2double(voltOffset(8:end-1));
sampleNum = sendread(osci, 'SANU? C1');
...
pkToPk = sendread(osci, 'C1:PAVA? PKPK');
pkToPk = str2double(pkToPk(13:end-1));

maxVoltage = sendread(osci, 'C1:PAVA? MAX');
maxVoltage = str2double(maxVoltage(12:end-1));

minVoltage = sendread(osci, 'C1:PAVA? MIN');
minVoltage = str2double(minVoltage(12:end-1));

meanVoltage = sendread(osci, 'C1:PAVA? MEAN');
